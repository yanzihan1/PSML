...
class _IONE(object):
    ...
    def build_g2(self,emd_size,neibor,ips0,ips1,ifps):
        def normalize_vector(vector):
            norm = tf.sqrt(tf.reduce_sum(tf.square(vector), 1, keep_dims=True))
            normalized_embeddings = vector / norm
            return normalized_embeddings
        self.pse1 = tf.placeholder(tf.float32, [None])
        self.sap = tf.placeholder(tf.float32, [None])
        ...
        self.embeddings = self.embeddings + tf.multiply(tf.nn.relu(new_add), ifps)
        self.pembedding1 = tf.nn.embedding_lookup(self.embeddings, self.pse1)
        self.sa = tf.nn.embedding_lookup(self.embeddings, self.sap)
        self.O=tf.reduce_sum(-1/(tf.exp(self.pse1,self.sa)))
        self.train_step_x2 = tf.train.GradientDescentOptimizer(1).minimize(self.O)
    ...
    def train_one_epoch(self):
        ...
        for batch in batches:
            ...
            _,cur_loss=self.sess.run([self.train_step_x2,self.O],feed_dict)
...
class IONE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=1000, epoch=10, negative_ratio=5, order=3, label_file=None, clf_ratio=0.5, auto_save=True):
        ...
        self.vectors = {}
        if order == 3:
            self.model1 = _IONE(graph, rep_size/2, batch_size,
                                negative_ratio, order=1)
            self.model2 = _IONE(graph, rep_size/2, batch_size,
            ...
            self.model = _IONE(graph, rep_size, batch_size,
