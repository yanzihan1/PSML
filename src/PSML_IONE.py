...
from graph import *
from src.openne import node2vec
from src.openne.classify import Classifier, read_node_label
from src.openne import Four
from src.openne import lap
from src.openne import gf
...
def parse_args():
    ...
    parser.add_argument('--input', default='../../data/IONE/T.edge',
                        help='Input graph file')
    parser.add_argument('--output',default='F0206.txt',
...
def main(args):
    ...
    if  args.method == 'line':
        if args.label_file and not args.no_auto_save:
            model = Four.IONE(g, epoch=args.epochs, rep_size=args.representation_size, order=args.order,
                              label_file=args.label_file, clf_ratio=args.clf_ratio)
        else:
            model = Four.IONE(g, epoch=args.epochs,
