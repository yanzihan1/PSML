...
class _IONE(object):
    ...
    def batch_iter(self):#
        ...
        for nod in edges:
            nod_le=nod[0]
            node_ri=nod[1]
        for fnode,fn in look_up.items():
            #fn=look_up.get(fnode)
            f[fn]=int(fnode)
        psnodes=[]
        for psnode in range(5313,5649):
            psnodes.append(look_up.get(str(psnode)))
        pnei=[]
        nodes=set()
        graph={}
        ...
        #for po in psnodes:
        #    pnei.append(random.choice(graph.get(pnei),20))
